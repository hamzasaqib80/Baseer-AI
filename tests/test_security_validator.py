...
def test_security_validator_domain_boundaries():
    ...
    """
    outlier_data = np.array([2.5, -1.2, 0.5]).reshape(3, 1, 1).repeat(32, axis=1).repeat(32, axis=2)
    sanitized = SecurityValidator.sanitize_input(outlier_data)
    
    assert torch.max(sanitized) <= 1.0
    assert torch.min(sanitized) >= 0.0

def test_malicious_payload_detection():
    """
    Ensures NaN and Inf injection attacks are blocked.
    """
    malicious_data = np.array([np.nan, np.inf, 0.5]).reshape(3, 1, 1).repeat(32, axis=1).repeat(32, axis=2)
