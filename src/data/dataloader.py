...
class CIFAR10DataLoader:
    ...
    def __init__(self, config: any):
        ...
        self.train_transform = transforms.Compose([
            transforms.RandomCrop(32, padding=self.config.dataset.augmentation.crop_padding),
            transforms.RandomHorizontalFlip(p=self.config.dataset.augmentation.horizontal_flip_prob),
            transforms.RandomRotation(self.config.dataset.augmentation.rotation_degrees),
            transforms.ToTensor(),
    ...
    def get_loaders(self) -> Tuple[DataLoader, DataLoader]:
        ...
        try:
            train_set = torchvision.datasets.CIFAR10(
                root='./data', train=True, download=not data_exists, transform=self.train_transform
            )
            
            test_set = torchvision.datasets.CIFAR10(
                root='./data', train=False, download=not data_exists, transform=self.test_transform
            )
        except Exception as e:
            print(f"CRITICAL: Failed to load dataset. If your local internet is timing out,")
            print(f"please copy the 'data/' folder from your Google Colab workspace into this directory.")
            raise e
