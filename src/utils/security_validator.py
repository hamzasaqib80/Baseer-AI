...
class SecurityValidator:
    ...
    def sanitize_input(
        ...
        if isinstance(data, np.ndarray):
            ...
            raise TypeError(
                f"Unsupported data type: {type(data)}. Only NumPy arrays and Torch Tensors are allowed."
            )
        ...
        if tensor.shape != torch.Size(expected_shape):
            raise ValueError(
                f"Input shape mismatch. Expected {expected_shape}, got {tensor.shape}"
            )

        # 3. Domain Boundary Sanitization
        if torch.isnan(tensor).any() or torch.isinf(tensor).any():
            raise ValueError(
                "Malicious payload detected: Input contains NaN or Inf values."
            )
