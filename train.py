...
class Trainer:
    ...
    def _init_device(self) -> torch.device:
        if torch.cuda.is_available():
            print(f"STATUS: Remote Compute Detected. Utilizing NVIDIA {torch.cuda.get_device_name(0)}")
    ...
    def train_epoch(self, epoch: int):
        ...
        print(f"Epoch {epoch:03d} | Loss: {avg_loss:.4f} | Acc: {acc:.2f}% | Time: {duration:.2f}s")
        return avg_loss, acc
    ...
    def run(self):
        print(f"INIZIALIZING TRAINING: {self.config.project.name} v{self.config.project.version}")
        ...
        for epoch in range(1, self.config.hyperparameters.epochs + 1):
            ...
            if val_acc > self.best_acc:
                ...
                if self.early_stop_counter >= self.config.hyperparameters.early_stopping.patience:
                    print(f"EARLY STOPPING: No improvement for {self.early_stop_counter} epochs.")
                    break
